...
export default function ProjectCardsList() {
  ...
  useEffect(() => {
    if (!isMusculiaLightboxOpen) return;
  ...
  return (
    <>
      <ul className="relative mx-auto w-full max-w-7xl list-none px-4 pt-6 pb-6 sm:px-6 sm:pt-8 sm:pb-8 lg:px-8 lg:pt-10 lg:pb-10">
        {PROJECT_IDS.map((id: ProjectId, index: number) => {
          const reverse = index % 2 === 1;
          const isCardDynatrust = id === "dynatrust";
          const coverCard = PROJECT_COVER[id];
          const title = t(`items.${id}.title`);
          const tagsJoined = splitTagsCsv(t(`items.${id}.tags`)).join(" · ");

          return (
            <li key={id} className="border-0">
              {index > 0 && (
                <div className="py-3 sm:py-4 md:py-5" aria-hidden>
                  <div className="mx-auto h-px w-full max-w-40 shrink-0 rounded-full bg-linear-to-r from-transparent via-[#0F6B78]/35 to-transparent sm:max-w-xs md:max-w-sm lg:max-w-md" />
                </div>
              )}
                  ...
                  >
                    ...
                    >
                      {coverCard ? (
                        ...
                        <div
                          className="absolute inset-0 bg-linear-to-br from-[#0F6B78]/28 via-zinc-100 to-white"
                          aria-hidden
                        />
                      )}
                    </figure>
                    ...
                    <div className="relative z-10 flex w-full flex-col justify-center bg-white py-4 sm:py-5 md:w-1/2 md:py-6 lg:py-8">
                      ...
                      <div className="mt-4 space-y-3 px-6 md:px-10 lg:px-14">
                        ...
                        <p className="text-sm text-zinc-500">{tagsJoined}</p>
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      type="button"
                      onClick={closeModal}
                      className="shrink-0 rounded-full border border-[#0F6B78]/30 bg-white/95 px-3 py-1.5 text-sm font-medium text-[#0F6B78] shadow-sm backdrop-blur hover:bg-[#f4fafb] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0F6B78]"
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5 sm:mt-4">
                    {splitTagsCsv(t(`items.${openId}.tags`)).map((tag, tagIdx) => (
                      <span
                        key={`${openId}-tag-${tagIdx}`}
                  ...
                  <div className="mt-4">
                        ...
                        return (
                          <button
                            ...
                            className={[
                              ...
                              "focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0F6B78]",
                              isActive
                                ? "border-[#0F6B78]/50 ring-2 ring-[#0F6B78]/20"
                                : "border-zinc-200 hover:border-[#0F6B78]/35",
                            ].join(" ")}
                            aria-label={t("galleryOpenImage", {index: idx + 1})}
                          >
                    ...
                    {isMusculiaLightboxOpen ? (
                      <div
                        ...
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="musculia-lightbox-title"
                      >
                        <div className="relative w-full max-w-5xl overflow-hidden rounded-2xl border border-white/15 bg-white shadow-2xl">
                          <div className="flex items-center justify-between gap-3 border-b border-zinc-200/80 bg-white px-4 py-3 sm:px-6">
                            <div className="min-w-0">
                              <p
                                id="musculia-lightbox-title"
                                className="text-sm font-bold tracking-[0.18em] text-[#0F6B78]"
                              >
                                {t("items.musculia.title")}
                              </p>
                            ...
                            <button
                              ...
                              className="shrink-0 rounded-full border border-[#0F6B78]/30 bg-white px-3 py-1.5 text-sm font-semibold text-[#0F6B78] shadow-sm transition hover:bg-[#f4fafb] focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#0F6B78]"
                              aria-label={t("galleryCloseLightbox")}
                            >
                              {t("modalClose")}
                            </button>
                          </div>
                          ...
                          <div className="relative bg-zinc-950">
                            ...
                            <button
                              ...
                              className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full border border-white/15 bg-white/90 px-3 py-2 text-sm font-semibold text-zinc-900 shadow-lg backdrop-blur hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0F6B78] sm:left-4"
                            ...
                            <button
                              ...
                              className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full border border-white/15 bg-white/90 px-3 py-2 text-sm font-semibold text-zinc-900 shadow-lg backdrop-blur hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0F6B78] sm:right-4"
                              ...
                              {musculiaGallery.map((src, idx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    aria-label={t("gallerySelectImage", {index: idx + 1})}
                                  >
                                    <div
                                      className={
                                        isActive
